...
async function msgHandler (client, message) {
    try {
        ...
        const commands = ['#buatsticker', '#buatstiker', '#halodimas', '#fb']
            ...
            if (!isGroupMsg) console.log(color('[EXEC]'), color(time, 'yellow'), color(cmd[0]), 'from', color(pushname))
                ...
                switch (cmd[0]) {
                    ...
                    if (isMedia) {
                        ...
                        client.sendText(from, 'Tidak ada gambar! Untuk membuat sticker kirim gambar dengan caption #buatstiker')
                    }
                    break
                    case '#halodimas':
                    client.sendText(from, 'Hai Teman Teman...')
                    break
                    case '#fb':
                    if (args.length == 2) {
                        const vidURL = args[1]
